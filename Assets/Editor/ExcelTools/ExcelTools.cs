...
public class ExcelTools : EditorWindow
{
    ...
    private void DrawTips()
    {
        ...
        EditorGUILayout.LabelField("使用说明：",GUILayout.Width(85));
        GUILayout.EndHorizontal();

        //GUILayout.BeginHorizontal();
        EditorGUILayout.LabelField("使用前请先创建一次ExcelSetting,里面记录导出路径，如没填写，默认导在对应excel文件夹");
        //GUILayout.EndHorizontal();
        EditorGUILayout.LabelField("该工具将导出json文件和对应的实体类，支持多sheet表导出，每个文件命名为对应的sheet表名字");
    }
    ...
    private static void Convert()
    {
        foreach (string excelPath in excelList)
        {
            ...
            if (indexOfFormat == 0)
            {
                ...
                string jsonPath = excelPath.Substring(0, excelPath.LastIndexOf("/"));
                string cellPath=jsonPath;
                jsonPath = string.IsNullOrEmpty(ExcelSetting.Instance.SaveJsonPath) ? jsonPath : ExcelSetting.Instance.SaveJsonPath;
                cellPath = string.IsNullOrEmpty(ExcelSetting.Instance.SaveCellPath) ? cellPath : ExcelSetting.Instance.SaveCellPath;
                nameSpace = ExcelSetting.Instance.NameSpace;
                excel.ConvertToJson(jsonPath,cellPath,nameSpace, encoding);
            }
    ...
    private static void Init()
    {
        ...
        if (ExcelSetting.Instance == null)
        {
            ExcelSetting.CreateExcelSetting();
        }
