...
//				mShaderFeatures = new List<string>(shaderImporter.userData.Split(new string[]{","}, System.StringSplitOptions.RemoveEmptyEntries));
	...
	public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
	{
		...
		return;
#else
		//此处做了修改，否则会出现  ArgumentException: Getting control 0's position in a group with only 0 controls when doing repaint
		//添加了当前事件的条件判定 Event.current.type != EventType.Layout ,即调用OnGUI时，不可空Return.. 否则会出问题
